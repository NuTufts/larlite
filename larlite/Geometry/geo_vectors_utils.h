...
namespace geo {
  ...
  namespace vect {
    ...
    using larlite::geo::Vector_t;
    using larlite::geo::Point_t;

    /// Convert the specified point into a `geo::Point_t`.
    template <typename Point>
    ::larlite::geo::Point_t toPoint(Point const& p)
      { return geo::vect::convertTo<::larlite::geo::Point_t>(p); }

    /// Convert the specified vector into a `geo::Vector_t`.
    template <typename Vector>
    ::larlite::geo::Vector_t toVector(Vector const& v)
      { return geo::vect::convertTo<::larlite::geo::Vector_t>(v); }
    ...
    GENVECTOR_CONSTEXPR ::larlite::geo::Point_t makePointFromCoords(Coords&& coords)
      { return makeFromCoords<::larlite::geo::Point_t>(std::forward<Coords>(coords)); }

    /// Creates a `geo::Vector_t` from its coordinates (see `makeFromCoords()`).
    template <typename Coords>
    GENVECTOR_CONSTEXPR ::larlite::geo::Vector_t makeVectorFromCoords(Coords&& coords)
      { return makeFromCoords<::larlite::geo::Vector_t>(std::forward<Coords>(coords)); }
...
namespace ROOT::Math {
  ...
  decltype(auto) begin(ROOT::Math::PositionVector2D<CoordSystem, Tag> const& v)
    { return larlite::geo::vect::vector_cbegin(v); }
  ...
  decltype(auto) end(ROOT::Math::PositionVector2D<CoordSystem, Tag> const& v)
    { return larlite::geo::vect::vector_cend(v); }
...
//---
namespace larlite::geo::vect {

  //----------------------------------------------------------------------------
  template <>
  inline auto norm(larlite::geo::Vector_t const& v) { return v.R(); }
...
namespace larlite::geo::vect::details {
  ...
  struct ConvertArrayTo: private ConvertToImplBase<Dest, Source>
  {
    ...
    static Dest convert(Source const& v)
      { return larlite::geo::vect::makeFromCoords<Dest>(v); }
  ...
  struct ConvertToImpl<Dest, T*> {
    ...
    static Dest convert(T* v) { return larlite::geo::vect::makeFromCoords<Dest>(v); }
...
//------------------------------------------------------------------------------
template <typename Vector>
constexpr std::array<std::size_t, larlite::geo::vect::dimension<Vector>()>
  larlite::geo::vect::indices()
{
  return details::makeIndexSeq<std::size_t, dimension<Vector>()>();
}

template <typename Vector>
constexpr auto larlite::geo::vect::indices(Vector const&) -> decltype(indices<Vector>())
  { return indices<Vector>(); }


//------------------------------------------------------------------------
template <typename Vector, typename Coords>
constexpr Vector larlite::geo::vect::makeFromCoords(Coords&& coords) {
  using namespace larlite::geo::vect::details;
  return makeFromCoordsImpl<Vector>
    (constexpr_forward<Coords>(coords), makeVectorIndices<Vector>());
} // larlite::geo::vect::makeFromCoords()


//------------------------------------------------------------------------------
template <typename Vector>
constexpr auto larlite::geo::vect::coordManager(unsigned int n)
  { return details::CoordManagerImpl<Vector>::get(n); }


//------------------------------------------------------------------------------
template <typename Vector>
auto larlite::geo::vect::coord(Vector& v, unsigned int n) noexcept {
...
//------------------------------------------------------------------------
template <typename Vector, typename Coords>
unsigned int larlite::geo::vect::fillCoords(Coords& dest, Vector const& src) {
  // this is simpler than makeFromCoords() because doesn't attempt constexpr
  for (std::size_t i = 0; i < larlite::geo::vect::dimension(src); ++i)
    dest[i] = larlite::geo::vect::coord(src, i);
  return larlite::geo::vect::dimension(src);
} // larlite::geo::vect::fillCoords()


//------------------------------------------------------------------------------
template <typename Vector>
constexpr auto larlite::geo::vect::coordManagers() {
  using PlainVector = std::remove_reference_t<Vector>;
  return
    details::CoordManagersImpl<PlainVector, dimension<PlainVector>()>::get();
} // larlite::geo::vect::coordManagers()

template <typename Vector>
constexpr auto larlite::geo::vect::coordManagers(Vector&&)
  { return coordManagers<Vector>(); }


template <typename Vector>
constexpr auto larlite::geo::vect::coordReaders() {
...
} // larlite::geo::vect::coordReaders()

template <typename Vector>
constexpr auto larlite::geo::vect::coordReaders(Vector&&)
  { return coordReaders<Vector>(); }


//------------------------------------------------------------------------------
template <typename Vector>
constexpr auto larlite::geo::vect::bindCoordManagers(Vector& v) {
  return details::BindCoordManagersImpl<Vector, dimension<Vector>()>::bind(v);
} // larlite::geo::vect::bindCoordManagers()


template <typename Vector>
constexpr auto larlite::geo::vect::bindCoordReaders(Vector const& v) {
  using ConstVector = std::add_const_t<std::remove_reference_t<Vector>>;
  return details::BindCoordManagersImpl<ConstVector, dimension<ConstVector>()>
    ::bind(v);
} // larlite::geo::vect::bindCoordReaders()


//------------------------------------------------------------------------------
template <typename Dest, typename Source>
Dest larlite::geo::vect::convertTo(Source const& v)
  { return details::ConvertToDispatcher<Dest, Source>::convert(v); }


//----------------------------------------------------------------------------
template <typename Dest, typename Source>
std::vector<Dest> larlite::geo::vect::convertCollTo(std::vector<Source> const& coll) {

  std::vector<Dest> dest;
  dest.reserve(coll.size());
  std::transform(coll.begin(), coll.end(), std::back_inserter(dest),
    larlite::geo::vect::convertTo<Dest, Source>);
  return dest;

} // larlite::geo::vect::convertCollTo()


//------------------------------------------------------------------------
template <typename Vector, typename Pred>
Vector larlite::geo::vect::transformCoords(Vector const& v, Pred&& pred) {
  details::CoordinateArray_t<Vector> values;
  unsigned int i = 0;
  for (auto c: bindCoordReaders(v)) values[i++] = pred(c());
  return makeFromCoords<Vector>(values);
} // larlite::geo::vect::transformCoords()


//------------------------------------------------------------------------------
template <typename Vector>
bool larlite::geo::vect::isfinite(Vector const& v)
