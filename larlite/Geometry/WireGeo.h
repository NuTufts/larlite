...
///
/// We've stripped the information down.
/// Mostly removed references to ROOT Geometry which populates this
/// the data in this class directly from GDML.
/// We will load from a data file to avoid the GDML parsing.
////////////////////////////////////////////////////////////////////////
...
#include "larlite/Geometry/LineClosestPoint.h"
...
namespace geo {
  ...
  public:
    ...
    WireGeo();
    WireGeo( float thetaz, float halflen, larlite::geo::Point_t& center );
     ...
     */
    template <typename Point = DefaultPoint_t>
    larlite::geo::IntersectionPointAndOffsets<Point> IntersectionAndOffsetsWith
      (larlite::geo::WireGeo const& other) const;
    ...
    /// (currently no-op)
    void UpdateAfterSorting(larlite::geo::WireID const&, bool flip);
    
    /// Returns the pitch (distance on y/z plane) between two wires, in cm
    static double WirePitch(larlite::geo::WireGeo const& w1, larlite::geo::WireGeo const& w2)
      { return std::abs(w2.DistanceFrom(w1)); }

  private:
    // using LocalTransformation_t = larlite::geo::LocalTransformationGeo
    ...
    larlite::geo::Point_t       fCenter;    ///< Center of the wire in world coordinates.
  ...
  static_assert(std::is_move_assignable_v<larlite::geo::WireGeo>);
  static_assert(std::is_move_constructible_v<larlite::geo::WireGeo>);
   ...
   * 
   */
  larlite::geo::Point_t WiresIntersection
    (larlite::geo::WireGeo const& wireA, larlite::geo::WireGeo const& wireB);
   ...
   * 
   * The returned intersection point is the same as for
   * `larlite::geo::WiresIntersection()`. The return value is actually triplet, though,
   * which is most easily unpacked immediately:
   * ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~{.cpp}
   * auto [ point, offsetA, offsetB ] = larlite::geo::WiresIntersection(wireA, wireB);
   * ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   * 
   * The two other elements of the triplets are the distances of the
   * intersection point from the center of this wire (`offset` in the example)
   ...
   * To reassign the variables after they have been defined, instead:
   * ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~{.cpp}
   * std::tie(point, offsetB, offsetA) = larlite::geo::WiresIntersection(wireB, wireA);
   * ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   * 
   */
  larlite::geo::IntersectionPointAndOffsets<larlite::geo::Point_t> WiresIntersectionAndOffsets
    (larlite::geo::WireGeo const& wireA, larlite::geo::WireGeo const& wireB);
...
//------------------------------------------------------------------------------
// template <typename Point>
// Point larlite::geo::Wirelarlite::geo::GetPositionFromCenterUnbounded(double localz) const {
//   return larlite::geo::vect::convertTo<Point>
//     (toWorldCoords(LocalPoint_t{ 0.0, 0.0, relLength(localz) }));
// } // larlite::geo::Wirelarlite::geo::GetPositionFromCenterImpl()


//------------------------------------------------------------------------------
template <typename Vector>
Vector larlite::geo::WireGeo::Direction() const {
  // maybe (GetCenter() - GetStart()) / HalfL() would be faster;
  // strangely, TVector3 does not implement operator/ (double).
  return larlite::geo::vect::convertTo<Vector>(GetEnd<larlite::geo::Point_t>() - GetStart<larlite::geo::Point_t>()) * (1.0 / Length());
} // larlite::geo::WireGeo::Direction()


//------------------------------------------------------------------------------
template <typename Stream>
void larlite::geo::WireGeo::PrintWireInfo(
  Stream&& out,
  std::string indent /* = "" */,
  ...
  //----------------------------------------------------------------------------
  out << "wire from " << GetStart<larlite::geo::Point_t>()
    << " to " << GetEnd<larlite::geo::Point_t>();
  ...
  //----------------------------------------------------------------------------
  out << "\n" << indent
    << "  center at " << GetCenter<larlite::geo::Point_t>() << " cm";

  if (verbosity-- <= 0) return; // 3

  //----------------------------------------------------------------------------
  out << ", direction: " << Direction<larlite::geo::Vector_t>();
...
//  if (verbosity-- <= 0) return; // 4
  ...
  //----------------------------------------------------------------------------
} // larlite::geo::WireGeo::PrintWireInfo()


//------------------------------------------------------------------------------
template <typename Point /* = DefaultPoint_t */>
Point larlite::geo::WireGeo::IntersectionWith(larlite::geo::WireGeo const& other) const {
  return larlite::geo::vect::convertTo<Point>(WiresIntersection(*this, other));
} // larlite::geo::WireGeo::IntersectionWith()


//------------------------------------------------------------------------------
template <typename Point /* = DefaultPoint_t */>
larlite::geo::IntersectionPointAndOffsets<Point> larlite::geo::WireGeo::IntersectionAndOffsetsWith
  (larlite::geo::WireGeo const& other) const
{
  auto const& [ point, ofsA, ofsB ] = WiresIntersectionAndOffsets(*this, other);
  return { larlite::geo::vect::convertTo<Point>(point), ofsA, ofsB };
} // larlite::geo::WireGeo::IntersectionAndOffsetsWith()


//------------------------------------------------------------------------------
inline larlite::geo::Point_t larlite::geo::WiresIntersection
  (larlite::geo::WireGeo const& wireA, larlite::geo::WireGeo const& wireB)
{
  
  return LineClosestPointWithUnitVectors(
    wireA.GetCenter<larlite::geo::Point_t>(), wireA.Direction<larlite::geo::Vector_t>(),
    wireB.GetCenter<larlite::geo::Point_t>(), wireB.Direction<larlite::geo::Vector_t>()
    );
  
} // larlite::geo::WiresIntersection()


//------------------------------------------------------------------------------
inline larlite::geo::IntersectionPointAndOffsets<larlite::geo::Point_t>
larlite::geo::WiresIntersectionAndOffsets
  (larlite::geo::WireGeo const& wireA, larlite::geo::WireGeo const& wireB)
{
